...
export function ProductsPage() {
  return (
    <div className="mx-auto max-w-6xl px-6 py-20">
      ...
      <p className="mb-3 max-w-lg text-sm text-gray-700">
        Reserve objects from the current rotation.
      </p>
      <p className="mb-16 max-w-lg text-xs leading-relaxed text-gray-600">
      ...
      <div className="grid gap-16 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          {pieces.map((item) => (
            ...
            >
              ...
              <div>
                <h3 className="text-lg font-light text-gray-800">{item.title}</h3>
                <p className="mt-1 text-xs text-gray-600">{item.note}</p>
                <p id="price" className="mt-3 text-sm text-gray-700">
                  From 4 200
                </p>
              </div>
            </article>
        ...
        <div className="rounded-sm border border-(--border-subtle) bg-(--bg-card) p-8">
          <h2 className="mb-8 text-xs tracking-[0.3em] text-gold-dark">INQUIRY</h2>
          <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
            <input
              type="text"
              name="name"
              placeholder="Full name"
              className="w-full border-0 border-b border-(--border-subtle) bg-transparent py-2 text-sm text-gray-800 placeholder:text-gray-600 outline-none"
            />
            <input
              type="email"
              name="email"
            ...
            <select
              name="piece"
              className="w-full border-0 border-b border-(--border-subtle) bg-transparent py-2 text-sm text-gray-800 outline-none"
            >
            ...
            <textarea
              ...
              placeholder="Notes"
              rows={3}
              className="w-full resize-none border-0 border-b border-(--border-subtle) bg-transparent py-2 text-sm text-gray-800 placeholder:text-gray-600 outline-none"
            />
            <button
              type="submit"
              className="ml-auto flex h-7 w-7 items-center justify-center rounded-full bg-gray-900 text-white"
            >
