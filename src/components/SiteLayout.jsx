...
const linkClass = ({ isActive }) =>
  `text-sm tracking-[0.2em] uppercase transition-colors duration-300 ${
    isActive ? 'text-(--gold)' : 'text-(--text-corrected) hover:text-(--mist-corrected)'
  }`
...
export function SiteLayout({ children }) {
  return (
    <div className="min-h-screen flex flex-col">
      <div className="border-b border-(--border-subtle) bg-(--bg-elevated)/80 backdrop-blur-md sticky top-0 z-50">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
          <NavLink to="/" className="group flex items-center gap-2">
            ...
            <span className="text-lg font-light tracking-[0.35em] text-(--text-corrected)">AURUM</span>
          </NavLink>
      ...
      <div className="border-t border-(--border-subtle) py-8 text-center text-xs text-(--text-corrected)">
